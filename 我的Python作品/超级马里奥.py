...
def create_map():
    ...
    for i in range(10):
        ...
        if not (location in locations):
            ...
            if obs_type == "tree": img = "shi.png"
            elif obs_type == "flag":  img = "jin.png"
...
while running:
    ...
    if hit:
        if hit[0].obs_type == "tree" and not hit[0].passed:
            points = points - 100
            skier.image = pygame.image.load("l.png")
            animate()
            pygame.time.delay(1000)
            skier.image = pygame.image.load("j.png")
            skier.angle = 0
            speed = [0, 6]
            hit[0].passed = True
